...
export class User extends AbstractModel {
    ...
    saveEmailCollection(data, type) {
        ...
        emailModel.saveEmail(data, type).then(() => {
        }, (e) => {
            console.log(e)
        });
    }
    ...
    sendData(data, type: USER_EMAIL_TEMPLATE) {
        const emailCollection = EmailCollection.find({
            email: data['email'],
            type: type,
    ...
    updateProfile(userId, userData: Object): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            if (!!userData['password']) {
                Accounts.setPassword(userData['_id'], userData['password'], {logout: false});
            }
            let user = OM.create<User>(User).loadById(userId);

            const assignDataToProfile = (fields: string[]) => {
                let userProfile = {};
                _.forEach(fields, (field) => {
                    if (userData["profile"].hasOwnProperty(field)) {
                        userProfile[field] = userData["profile"][field];
                    }
                });
                return userProfile;
            };

            if (userData.hasOwnProperty('pin_code') && userData.hasOwnProperty('pin_code') && userData.hasOwnProperty('code_information')) {
                SupportToken.checkDuplicate(userData, userData['pin_code'], userData['bar_code']);
                SupportToken.updateCodeLogin(user.getData(), userData['_id'], userData['pin_code'], userData['bar_code'], userData['code_information']);
            }

            const userProfile = assignDataToProfile(['first_name', 'last_name', 'phone', 'lang']);
            user.setData('profile', Object.assign({}, user.getProfile(), userProfile))
                .save()
                .then(() => {
                    return resolve();
